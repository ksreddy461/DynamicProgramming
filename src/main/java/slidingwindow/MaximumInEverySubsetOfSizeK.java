...
public class MaximumInEverySubsetOfSizeK {
    ...
    public int[] solve(int[] nums, int k) {
        ...
        int i = 0, j = 0;
        while (j < n) {
            while (!deQueue.isEmpty() && deQueue.peek().getElement() < nums[j]) {
                deQueue.remove();
            }
            deQueue.add(new Pair(nums[j], j));

            if (j - i + 1 == k) {
                if (deQueue.isEmpty()) {
                    res[i] = -1;
                } else {
                    res[i] = deQueue.peek().getElement();
                    if (res[i] == nums[i] && i == deQueue.peek().getPosition()) {
                        deQueue.remove();
                    }
                    i++;
                }
    ...
    public class Pair implements Comparable<Pair> {
        ...
        Pair(Integer element, int position) {
    ...
    public int[] solve2(int[] nums, int k) {
        int n = nums.length;
        int[] res = new int[n - k + 1];
        PriorityQueue<Pair> deQueue = new PriorityQueue<>(Collections.reverseOrder());
        ...
        while (j < n) {
            ...
            if (j - i + 1 == k) {
                while (deQueue.peek().getPosition() < i) {
                    deQueue.poll();
                }
                res[i] = deQueue.peek().getElement();
                deQueue.remove(new Pair(res[i],i));
                i++;
            }
            j++;
        }
